#!/usr/bin/env python2
# -*- coding: utf-8 -*- #
#Author: https://github.com/thricedotted/twitterbot/
...
class TwitterBot:
    ...
    def __init__(self):
        ...
        self.state['last_follow_check'] = float()
    ...
    def bot_init(self):
        ...
        raise NotImplementedError(
            "You MUST have bot_init() implemented in your bot! What have you DONE!")
    ...
    def _log_tweepy_error(self, message, e):
        try:
            ...
            self.log("{}: {} ({})".format(message, e_message, code),
                     level=logging.ERROR)
        except:
            self.log(message, e)
    ...
    def on_follow(self, f_id):
        ...
        """
        if self.config['autofollow'] == True:
            try:
    ...
    def _ignore_method(self, method):
        return hasattr(method,
                       'not_implemented') and method.not_implemented
    ...
    def _check_mentions(self):
        ...
        try:
            current_mentions = self.api.mentions_timeline(
            ...
            logging.info(
                'Mentions updated ({} retrieved, {} total in queue)'.format(
        ...
        #     self.log(
        #         'Incomplete read error -- skipping mentions update')
    ...
    def _check_timeline(self):
        ...
        try:
            current_timeline = self.api.home_timeline(count=50,
                                                      since_id=
                                                      self.state[
                                                          'last_timeline_id'])
            ...
            current_timeline = [t for t in current_timeline if
                                t.author.screen_name.lower() != self.screen_name.lower()]

            # remove all tweets mentioning me
            current_timeline = [t for t in current_timeline if
                                not re.search('@' + self.screen_name,
                                              t.text,
                                              flags=re.IGNORECASE)]

            if self.config['ignore_timeline_mentions']:
                # remove all tweets with mentions (heuristically)
                current_timeline = [t._json for t in current_timeline if
                                    '@' not in t.text]
            ...
            self.state['recent_timeline'] = list(
                reversed(current_timeline))

            logging.info('Timeline updated ({} retrieved)'.format(
                len(current_timeline)))
        ...
        # except IncompleteRead as e:
        #     self.log(
        #         'Incomplete read error -- skipping timeline update')
    ...
    def _check_followers(self):
        ...
        try:
            self.state['new_followers'] = [f_id for f_id in
                                           self.api.get_follower_ids(
                                               user_id=self.id) if
                                           f_id not in self.state[
                                               'followers']]
        ...
        # except IncompleteRead as e:
        #     self.log(
        #         'Incomplete read error -- skipping followers update')
    ...
    def run(self):
        ...
        if self.config["autofollow"] and (time.time() - self.state['last_follow_check']) > (1 * 60):
        # if self.state['last_follow_check'] > (1 * 60):
        ...
        if (time.time() - self.state['last_tweet_time']) > \
                self.config['tweet_interval']:
            ...
            if self.config['tweet_interval_range'] is not None:
                self.config['tweet_interval'] = random.randint(
                    *self.config['tweet_interval_range'])

            self.log("Next tweet in {} seconds".format(
                self.config['tweet_interval']))
            self.state['last_tweet_time'] = time.time()

        # run custom action
        for handler in self.custom_handlers:
            if (time.time() - handler['last_run']) > handler[
                'interval']:
                handler['action']()
